...
This project **MUST** be hosted on an externally accessable url as required by Azure AD B2C in order to access metadata and Jwks.
...
And then upload to your custom policies to Azure Ad B2C in the Identity Framework section in the following order:
...
### Creating a token
To create an exchangable token, call the `IOidcService.BuildSerializedIdTokenAsync(string audience, int duration, string userEmail)` endpoint which will generate a JWT token with the given email as a claim. 
...
Please do not open any github issues unless you can demonstrate a clear, reproducable bug. If you are simply unable to get the project up and running, please check our upcoming blog posts found at https://www.muljin.com/blog or our youtube channel. Tutorials will be linked to this readme as they are released.
